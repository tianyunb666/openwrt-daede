...
# 👇 【中文注释：这里是官方原有的 GitHub 加速反代前缀】
GITHUB_PROXY_PREFIX="${GITHUB_PROXY_PREFIX:-https://ghfast.top/}"

# 👇 【中文注释：核心修改点！在此处为 GITHUB_API_URL 加上加速前缀变更为 ${GITHUB_PROXY_PREFIX}，确保国内路由能正常获取到内核最新版本号】
GITHUB_API_URL="${GITHUB_PROXY_PREFIX}https://api.github.com/repos/kenzok8/openwrt-daede/releases/latest"
...
# 👇 【中文注释：第二处微调。将内核内核所需的 BTF 固件的接口地址，同样加上原有的加速前缀，防止因 kernel 架构不对走 Fallback 流程时卡在 BTF 检测上】
VMLINUX_BTF_API="${VMLINUX_BTF_API:-${GITHUB_PROXY_PREFIX}https://api.github.com/repos/kenzok8/vmlinux-btf/releases/tags/latest}"
